...
module.exports = function(app, config, coreLibs){
  ...
  var pageAppBundleHandler = async.simpleCompose(app.pluginsMap.pageAppBundle);
  ...
  var go = function(page){
    ...
    if(page.app){
      ...
      pageAppBundleHandler({
        page:page,
        bundle: bundle
      }, function(err, env){
        app.pluginsMap.bundles.push(env.bundle);
        var bundleRoute = path.normalize((config.bundlesOptions.prefix || "/bundles/")+env.bundle.mountPoint);
        if(Array.isArray(env.page.javascripts)){
          env.page.javascripts.push(bundleRoute);
        }
        else{
          env.page.javascripts = [bundleRoute];
        }

        // Extract routes from app router
        var approuter = require(env.bundle.entryPoint);
        env.page.route = _.keys(approuter.routes).map(function(route){
          if(route=="/"){
            return "/"+env.page._pagename;
          }
          else{
            return path.normalize("/"+env.page._pagename+"/"+route);
          }
        });
      });
