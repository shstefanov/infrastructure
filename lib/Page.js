...
var Page = Class.extend("Page", {
  ...
  constructor: function(env){
    ...
    if(this.pre) {
      if(Array.isArray(this.pre)){
        for(var i=0;i<this.pre.length; i++){
          if(_.isString(this.pre[i])){
            app.all(this.root+"*", this.createCallFromString(this.pre[i]));
          }
          else if(_.isFunction(this.pre[i])){
            app.all(this.root+"*", this.pre[i]);
          }
          else if(_.isObject(this.pre[i])){
            app.all(this.root+"*", this.createParallelCall(this.pre[i], true));
          }
        }
      }
      else{
    ...
    if(this.after) {
      if(Array.isArray(this.after)){
        for(var i=0;i<this.after.length; i++){
          if(_.isString(this.after[i])){
            app.all(this.root+"*", this.createCallFromString(this.after[i]));
          }
          else if(_.isFunction(this.after[i])){
            app.all(this.root+"*", this.after[i]);
          }
          else if(_.isObject(this.after[i])){
            app.all(this.root+"*", this.createParallelCall(this.after[i], true));
          }
        }
      }
      else{
  ...
  createCallFromString: function(target, multi){
    ...
    var customAttributes, resultName;
    if(target.indexOf("|")!==-1){
    ...
    if(target.indexOf(".")===-1){
      ...
      var getAttributes;
      if(customAttributes){
        getAttributes = new Function("req, res, _", "return ["+customAttributes+"];");
      }
      ...
      return multi? (resultName? function(req, res, next){
          var attributes = getAttributes(req, res, _);
          env.i.do.apply(env.i, [target].concat(attributes).concat([ function(err, result){
            if(err) return next(err);
            res.data = res.data || {};
            res.data[resultName] = result;
            next();
          }]));
      ...
      :( resultName? function(req, res, next){
          var attributes = getAttributes(req, res, _);
          env.i.do.apply(env.i, [target].concat(attributes).concat([function(err, result){
            if(err) return next(err);
            res.data = res.data || {};
            res.data = _.extend(res.data||{}, _.object([[resultName, result]]));
            self.render(req, res);
          }]));
        } : function(req, res, next){
          var attributes = getAttributes(req, res, _);
  ...
  createParallelCall: function(target, isPartOfChain){
    ...
    var targetsIterators = _.keys(target).map(function(address){
      ...
      return function(req, res, cb){
        ...
        env.i.do.apply(env.i, [address].concat([attributes]).concat([callback]));
      }
