...
var Page = Class.extend("Page", {
  ...
  constructor: function(env){
    ...
    if(this.pre) {
      ...
      env.call("log.sys", ["route", "ALL"+line.slice("ALL".length)+ this.root+"*"])
    }
    for(var key in this){
      if(key.match(/^GET\s|^POST\s|^PUT\s|^DELETE\s|^ALL\s/)){
        (function(key){
          ...
          env.call("log.sys", ["route", method.toUpperCase()+line.slice(method.length)+ route]);
    ...
    if(target.indexOf(".")===-1){
      ...
      if(customAttributes){
        getAttributes = function(req, res){
          return eval("("+customAttributes+")");
        }
      }
      else if(_.isFunction(self.callAttributes)){
        getAttributes = function(req, res){
          return self.callAttributes(req, res);
        }
      }
      else{
