...
module.exports = function(){
  ...
  _.extend(env, {
    ...
    sys: function(logName, value){
      ...
      var date = new Date().toISOString().replace(/\..*$/, "").replace("T", " ");
      var id = this.config.id? "["+this.config.id+"]":"[main]";
      console.log("["+date+"]"+id+(this.config.address?("["+this.config.address+"]"+shortline.slice(this.config.address.length)):"")+"["+logName+"]"+line.slice(logName.length), value);
    }
  });
