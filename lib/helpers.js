...
var helpers = module.exports = {
  ...
  amap: function(arr, iterator, cb, ctx){
    ...
    if(_.isArray(iterator)) {
      if(_.every(iterator, _.isFunction)) iterator = helpers.chain(iterator, ctx);
      else {
        extraargs = iterator.slice();
        // iterator = function(){console.log("ITERATOR::", arguments);}
      }
    }
    var results, counter;
    ...
    arr.forEach(function(el, idx, arr){
      setTimeout(function(){
        (extraargs? el : iterator).apply(ctx, (extraargs || [el]).concat([function(err, result){
  ...
  objectWalk: function(obj, iterator){
    for(var key in obj){
      (function(name, val){
        if(_.isObject(val) && (typeof val !== "function")){
          return helpers.objectWalk(val, iterator);
        }
        iterator(name, val, obj);
      })(key, obj[key]);
    }
