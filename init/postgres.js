...
module.exports = function(cb){
  ...
  pg.connect(connectURL, function(err, client, release) {
    ...
    env.call("log.sys", ["postgres", "Connected to MongoDB on "+(config.postgres.host || "localhost")+":"+(config.postgres.port||5432)+"/"+(config.postgres.database||config.postgres.db)] );
    cb();
  });
