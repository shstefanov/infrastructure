...
module.exports = function(app, socket, session){
  ...
  var DefineService = function(definition){
    ...
    var modelPermissions = session.permissions[definition.name] || 
    app.config.permissions.roles[session.role][definition.name] || 
    app.config.permissions.defaults[definition.name];

    var service = {
      name:definition.name, 
      modelDefinition:definition,
      permissions: definition.__permissions
    };
  ...
  var enabledModelServices = session.modelServices;
  ...
  if(typeof modelsDefinition == "function"){
    ...
    //Parse model definition function to get model definitions
    modelsDefinition(dbArgument, AttributeTypes, function(modelsHash){
      enabledModelServices.forEach(function(service){
        ...
        //Check if is valid model name
        if(!app.models[serviceName]) {
          throw new Error("Can't build model service "+serviceName+" No such model");
          return;
        }
        modelsHash[serviceName].__permissions = service.permissions;
        //old code
